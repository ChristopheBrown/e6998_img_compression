...
flags = tf.app.flags
flags.DEFINE_string('path', 'C:\\Users\\Donis\\Documents\\compression\\residual-rnn\\data\\data\\', 'dataset path')
...
flags.DEFINE_integer('iters', 20, 'number of iterations of model')
...
with tf.device('/cpu:0'):
    ...
    iterator = tf.data.Iterator.from_structure(dataset.data.output_types, dataset.data.output_shapes)
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
    ...
    print("Start training... step per epoch: {}".format(batches_per_epoch))
    ...
    for epoch in range(opts.epochs):
        ...
        for step in range(batches_per_epoch):
            ...
            if (step + 1) % opts.display == 0:
                ...
                print("Epoch number: {} step {} loss {}".format(epoch + 1, step + 1, tmp_loss))
                tmp_loss = 0

        
        compressed_img = compressed.eval(feed_dict={model.inputs: img_batches}).clip(0, 255).astype(np.uint8)
